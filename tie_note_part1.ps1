...
$c_f_lib_payoffSimulate_ts = Select-String -Path "lib/payoffSimulate.ts" -Pattern "strategiesTie" -SimpleMatch
...
$c_f_app_components_AmortizationSchedule_tsx = Select-String -Path "app/components/AmortizationSchedule.tsx" -Pattern "Both strategies give the same result" -SimpleMatch
...
if ($global:anyFail) {
    ...
    Write-Host ""
    Write-Host "Part 1 done. Now run tie_note_part2.ps1" -ForegroundColor Cyan
}