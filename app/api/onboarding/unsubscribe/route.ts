...
function page(title: string, message: string): string {
  return (
    ...
    "</h1><p>" +
    message +
    "</p><p><a href=\"/dashboard\" style=\"color:#34d399;\">Back to your dashboard</a></p>" +
    "</div></body></html>"
  )
}
...
export async function GET(req: Request) {
  ...
  const { data, error } = await db
    .from("profiles")
    .update({ onboarding_unsubscribed: true })
    .eq("onboarding_unsub_token", token)
    .select("email")
    .maybeSingle()
  ...
  return new NextResponse(
    page("You're unsubscribed", "You won't get any more setup tips -- account and billing emails are unaffected."),
