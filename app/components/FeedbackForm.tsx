...
import { ThumbsUp, ThumbsDown, Loader2, CheckCircle2, MessageSquare, Lightbulb } from "lucide-react"
import { supabase } from "@/lib/supabase/client"

type Sentiment = "like" | "dislike" | null
// "feature_request" lets Vince filter the admin feedback list down to just
// what people are asking to be built, instead of it all being lumped in
// with general like/dislike notes (Vince, Aug 27 2026).
type Kind = "feedback" | "feature_request"
...
export default function FeedbackForm({ onDone }: { onDone?: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const { error } = await supabase.from("feedback").insert({
        ...
        type: kind,
        // Sentiment only makes sense for general feedback -- a feature
        // request isn't a thumbs-up/down on something that exists yet.
        sentiment: kind === "feedback" ? sentiment : null,
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="mb-2 text-sm text-gray-300">What's this about?</p>
      ...
      <textarea
        ...
        placeholder={
          kind === "feature_request"
            ? "What would you like to see us build?"
            : "What do you like, or what would make this better?"
        }
