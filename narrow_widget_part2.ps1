...
$c_f_app_debts_page_tsx = Select-String -Path "app/debts/page.tsx" -Pattern "Extra monthly payment" -SimpleMatch
...
foreach ($f in @("lib/payoffSimulate.ts","app/components/AmortizationSchedule.tsx")) {
    if (-not (Test-Path $f)) { Write-Host "FAIL $f missing - did you run narrow_widget_part1.ps1 first?" -ForegroundColor Red; $allGood = $false }
}
if (-not $allGood) {
    Write-Host ""
    Write-Host "Run narrow_widget_part1.ps1 first, then part2." -ForegroundColor Red
    exit 1
}
...
git commit -m "Replace full Payoff Plan block on Debts page with compact Strategy/Extra widget"
