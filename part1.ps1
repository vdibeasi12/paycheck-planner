...
'@
$targetPath_payoffSimulate = "lib/payoffSimulate.ts"
$dir_payoffSimulate = Split-Path $targetPath_payoffSimulate -Parent
if ($dir_payoffSimulate -and -not (Test-Path $dir_payoffSimulate)) { New-Item -ItemType Directory -Path $dir_payoffSimulate -Force | Out-Null }
[System.IO.File]::WriteAllText($targetPath_payoffSimulate, $payoffSimulate, (New-Object System.Text.UTF8Encoding($false)))
$check_payoffSimulate = Select-String -Path $targetPath_payoffSimulate -Pattern 'export type AvalancheCriterion' -SimpleMatch
if (-not $check_payoffSimulate) { Write-Host "FAIL: lib/payoffSimulate.ts did not contain expected content" -ForegroundColor Red; $global:anyFail = $true } else { Write-Host "OK: lib/payoffSimulate.ts verified" -ForegroundColor Green }

# ---- app/components/AmortizationSchedule.tsx ----
...
'@
$targetPath_AmortizationSchedule = "app/components/AmortizationSchedule.tsx"
$dir_AmortizationSchedule = Split-Path $targetPath_AmortizationSchedule -Parent
if ($dir_AmortizationSchedule -and -not (Test-Path $dir_AmortizationSchedule)) { New-Item -ItemType Directory -Path $dir_AmortizationSchedule -Force | Out-Null }
[System.IO.File]::WriteAllText($targetPath_AmortizationSchedule, $AmortizationSchedule, (New-Object System.Text.UTF8Encoding($false)))
$check_AmortizationSchedule = Select-String -Path $targetPath_AmortizationSchedule -Pattern 'Biggest balance' -SimpleMatch
if (-not $check_AmortizationSchedule) { Write-Host "FAIL: app/components/AmortizationSchedule.tsx did not contain expected content" -ForegroundColor Red; $global:anyFail = $true } else { Write-Host "OK: app/components/AmortizationSchedule.tsx verified" -ForegroundColor Green }

if ($global:anyFail) {
    Write-Host "One or more files failed verification in Part 1. Stopping before git." -ForegroundColor Red
    exit 1
}

Write-Host "Part 1 complete. Now run the next part." -ForegroundColor Cyan