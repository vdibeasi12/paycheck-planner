# Marketing attribution part 2 (final)
[Environment]::CurrentDirectory = (Get-Location).Path
$ErrorActionPreference = "Stop"
$global:anyFail = $false

$f_app_admin_page_tsx = @'
"use client";
...
'@
$dir_f_app_admin_page_tsx = Split-Path "app/admin/page.tsx" -Parent
if ($dir_f_app_admin_page_tsx -and -not (Test-Path $dir_f_app_admin_page_tsx)) { New-Item -ItemType Directory -Path $dir_f_app_admin_page_tsx -Force | Out-Null }
[System.IO.File]::WriteAllText((Join-Path (Get-Location) "app/admin/page.tsx"), $f_app_admin_page_tsx, (New-Object System.Text.UTF8Encoding($false)))
$c_f_app_admin_page_tsx = Select-String -Path "app/admin/page.tsx" -Pattern "Traffic source (auto-detected)" -SimpleMatch
if ($c_f_app_admin_page_tsx) { Write-Host "OK   app/admin/page.tsx" -ForegroundColor Green } else { Write-Host "FAIL app/admin/page.tsx" -ForegroundColor Red; $global:anyFail = $true }
...
$allGood = $true
foreach ($f in @("app/components/AttributionCapture.tsx","app/layout.tsx","app/auth/callback/route.ts","app/api/admin/users/route.ts")) {
    if (-not (Test-Path $f)) { Write-Host "FAIL $f missing - did you run attribution_part1.ps1 first?" -ForegroundColor Red; $allGood = $false }
}
if (-not $allGood) {
    Write-Host ""
    Write-Host "Run attribution_part1.ps1 first, then part2." -ForegroundColor Red
    exit 1
}
...
git add "app/components/AttributionCapture.tsx" "app/layout.tsx" "app/auth/callback/route.ts" "app/api/admin/users/route.ts" "app/admin/page.tsx"
git commit -m "Add automatic marketing attribution (UTM/referrer) tracking to Admin Panel"
