# Replaces the select()-based fix (unreliable on type=number inputs in some
# browsers) with a bulletproof clear-on-focus approach for the Extra Monthly
# Payment field on both the Payoff Plan and standalone Debt Payoff Calculator.
...
$c_f_app_components_AmortizationSchedule_tsx = Select-String -Path "app/components/AmortizationSchedule.tsx" -Pattern "extraText" -SimpleMatch
...
$c_f_app_debt_payoff_calculator_page_tsx = Select-String -Path "app/debt-payoff-calculator/page.tsx" -Pattern "extraPaymentText" -SimpleMatch
...
git commit -m "Replace unreliable select() fix with a clear-on-focus pattern for the extra payment field"
