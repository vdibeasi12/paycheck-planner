...
$c_f_lib_payoffSimulate_ts = Select-String -Path "lib/payoffSimulate.ts" -Pattern "export function simulate" -SimpleMatch
...
$c_f_app_components_AmortizationSchedule_tsx = Select-String -Path "app/components/AmortizationSchedule.tsx" -Pattern "payoffSimulate" -SimpleMatch
...
if ($global:anyFail) {
    ...
    Write-Host ""
    Write-Host "Part 1 done. Now run narrow_widget_part2.ps1" -ForegroundColor Cyan
}